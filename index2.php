...
function printUsers () {
    ...
    foreach ($users as $u) {
        echo "<tr>
            ...
            <td>
                <a href='index.php?action=rm&id={$u['user_id']}'>rm</a> ::
                <a href='index.php?action=mod&id={$u['user_id']}'>ed</a>
            </td>
        </tr>";
    }
}
...
$db = new Mysqlidb ('localhost', 'root', '', 'circ');
if ($_GET) {
...
<!--        <input type=hidden name='id' value='--><?php //echo $data['user_id']?><!--'>-->
        <input type=text name='login' required placeholder='Login' value='<?php // echo $data['email']?>'>
